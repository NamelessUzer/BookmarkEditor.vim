...
" 定义书签不同层级的正则表达式匹配规则
syntax match NonMain "^\s*\zs\(封皮\|封面\|书名\|扉页\|版权\|[总分]\?目录\|简目\|详目\|[前导弁引]言\|序[文言]\?\|[前自再代总]序\|代总序\|绪论\|编者的话\|作者简介\|\|关于作者\|作者\|摘要\|关键词\|缩略语\|术语表\|附录\|索引\|后\?附录\|内容摘要\|法律声明\|读者对象\|注释\|结语\|后记\|封底\)页\?\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\s*\)\?$"
syntax match NonMain "^\s*\zs\(\d\{4}\s\?[年版]\?\)\([前引]言\|序言\?\)\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\s*\)\?$"
