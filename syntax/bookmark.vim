...
syntax match NonMain "^\s*\zs\(作者简介\|关于作者\|作者\)页\?\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\(\s\/XYZ\s\d\+\(\.\d\+\)\?\s\d\+\(\.\d\+\)\?\s\d\+\(\.\d\+\)\?\)\?\)$"
syntax match NonMain "^\s*\zs\(索引\|后\?附录\|结语\|后[记序题]\|封底\)页\?\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\(\s\/XYZ\s\d\+\(\.\d\+\)\?\s\d\+\(\.\d\+\)\?\s\d\+\(\.\d\+\)\?\)\?\)$"
syntax match NonMain "^\s*\zs\(参考\(文献\|书目\)\)页\?\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\(\s\/XYZ\s\d\+\(\.\d\+\)\?\s\d\+\(\.\d\+\)\?\s\d\+\(\.\d\+\)\?\)\?\)$"
