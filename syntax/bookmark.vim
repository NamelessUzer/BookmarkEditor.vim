...
" 定义书签不同层级的正则表达式匹配规则
syntax match NonMain "^\s*\zs\(扉页\|跋\)\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\s*\)\?$"
syntax match NonMain "^\s*\zs\(封皮\|封面\|书名\|版权\(信息\)\?\)页\?\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\s*\)\?$"
syntax match NonMain "^\s*\zs\([总分]\?目录\|简目\|详目\)页\?\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\s*\)\?$"
syntax match NonMain "^\s*\zs\(\d\{4}\s*年\?版\?\)\?\([前自再代总]序\|[前导弁引]言\|序[文言]\?\|代总序\)页\?\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\s*\)\?$"
syntax match NonMain "^\s*\zs\(绪论\|使用指南\|编者的话\|摘要\|内容摘要\|关键词\|缩略语\|术语表\|法律声明\|读者对象\|注释\)页\?\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\s*\)\?$"
syntax match NonMain "^\s*\zs\(作者简介\|关于作者\|作者\)页\?\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\s*\)\?$"
syntax match NonMain "^\s*\zs\(索引\|后\?附录\|结语\|后[记序题]\|封底\)页\?\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\s*\)\?$"
syntax match NonMain "^\s*\zs\(参考\(文献\|书目\)\)页\?\ze\(\(\s\|[.⋯/／]\)*-\?\d\+\s*\)\?$"
...
syntax match BookmarkSubpart "^\s*\zs第\s*\([零一二三四五六七八九十百千万]\+\|\d\+\)\s*分\?编.\{-}\ze\([.⋯\s/／]*-\?\d\+\s*\)$"
syntax match BookmarkChapter "^\s*\zs第\s*\([零一二三四五六七八九十百千万]\+\|\d\+\)\s*\([章讲回]\|单元\).\{-}\ze\([.⋯\s/／]*-\?\d\+\s*\)$"
